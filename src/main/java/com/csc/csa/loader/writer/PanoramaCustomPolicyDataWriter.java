...
public class PanoramaCustomPolicyDataWriter implements ItemWriter<Party> {
	...
	public void checkForClientDeletes() {
		...
		if (clientListToDelete != null && !clientListToDelete.isEmpty()) {
			...
			List<String> portList = jdbcTemplate.getJdbcOperations()
					.queryForList(Constants.PORT_RETRIEVE_ON_CLIENTID_SQL + whereDeleteClause, String.class);
			...
			if (!CollectionUtils.isEmpty(clientListToDelete)) {
				...
				List<String> addressIDToDelete = jdbcTemplate.getJdbcOperations()
						.queryForList(Constants.RETRIEVE_ADDRID_TODELETE_IN_SQL + whereDeleteClause, String.class);
	...
	public void insertPartyData(Party party) {
		...
		// CSCOPT-1743 begin - delete any preexisting addresses so only latest
		// one will be on database
		// if(partyPresent &&
		// !CollectionUtils.isEmpty(party.getClientAddrList())){//CSCOPT-1842,
		// 1985
	 ...
	 * 
	 * @param party
	 * @return Getting client Id from client.host_client table
	 */
	...
	protected void deletePolicyData(Party party) {
		...
		// this client id list is needed later to delete client ids no longer on
		// the portfolio table
	...
	// CSCOPT-1985 end
	@Transactional(propagation = Propagation.REQUIRED)
	public void deletePartyData(Party party) {
		deleteProducerContractData(party.getPortfolio());
		List<String> clientIDList = retrieveClientIDList(party);
		if (clientIDList != null && !clientIDList.isEmpty()) {
	...
	public List<String> retrieveClientIDList(Party party) {
		SqlParameterSource namedParameters = new BeanPropertySqlParameterSource(party.getPortfolio());
		List<String> val = jdbcTemplate.queryForList(Constants.PORT_RETRIEVE_ON_CONTRACTID_SQL, namedParameters,
				String.class);
		return val;
	}
	...
	protected String checkIfPartyPresent(Party party) {
		SqlParameterSource namedParameters = new BeanPropertySqlParameterSource(party);
		List<String> val = jdbcTemplate.queryForList(Constants.PARTY_RETRIEVE_SQL, namedParameters, String.class);
	...
	/**
	 * This method insert party data into host_portfolio table so that in csA ,
	 * user can search by policy number
	 */
	...
	/**
	 * This method insert party data into host_client table so that in csA
	 * Address is visible
	 * 
	 * @param party
	 */
	...
	public void insertParty(Party party) {
		party.setHostID("7");
		SqlParameterSource partyDataForSqlQuery = new BeanPropertySqlParameterSource(party);
	...
	public void insertAgentPortfolio(Party party) {
		...
		if (!processedPolicyList.contains(party.getPortfolio().getContractID())) {
			SqlParameterSource portRetrieve = new BeanPropertySqlParameterSource(party.getPortfolio());
			List<String> val = jdbcTemplate.queryForList(Constants.PORT_RETRIEVE_SQL, portRetrieve, String.class);
			if (val != null && !val.isEmpty()) {
				party.getPortfolio().setClientID(party.getClientID());
				party.getPortfolio().setClientContractID(val.get(0));
				SqlParameterSource prodPortRetrieve = new BeanPropertySqlParameterSource(party.getPortfolio());
				int val1 = jdbcTemplate.queryForObject(Constants.RETRIEVE_PRODUCER_CONTRACT_SQL, prodPortRetrieve,
						Integer.class);
				if (val1 == 0) {
					...
					SqlParameterSource portParam = new BeanPropertySqlParameterSource(port);
					jdbcTemplate.update(Constants.INSERT_PRODUCER_CONTRACT_SQL, portParam);
				}
			}
		} else {
	...
	public String checkIfAgentPresent(Party party) {
		SqlParameterSource namedParameters = new BeanPropertySqlParameterSource(party);
		List<String> val = jdbcTemplate.queryForList(Constants.AGENT_RETRIEVE_SQL, namedParameters, String.class);
		if (val == null || val.isEmpty()) {
			return null;
		}
		return val.get(0);
	...
	public void insertAddress(List<Address> addrList) {
		SqlParameterSource[] params = SqlParameterSourceUtils.createBatch(addrList.toArray());
		jdbcTemplate.batchUpdate(Constants.INSERT_ADDR_SQL, params);
	}

	public void insertClientAddress(Party party) {
		SqlParameterSource[] params = SqlParameterSourceUtils.createBatch(party.getClientAddrList().toArray());
	...
	public void deleteAllData(SqlParameterSource params, String whereClause, Portfolio port,
		...
		try {
			...
			jdbcTemplate.getJdbcOperations().update(Constants.DELETE_PORT_SQL + whereClause2);
			List<String> portList = jdbcTemplate.getJdbcOperations()
					.queryForList(Constants.PORT_RETRIEVE_ON_CLIENTID_SQL + whereClause, String.class);
			clientIDList.removeAll(portList);
			if (!CollectionUtils.isEmpty(clientIDList)) {
				whereClause = buildInClause(clientIDList); // CSCOPT-1985
				List<String> addressIDToDelete = jdbcTemplate.getJdbcOperations()
						.queryForList(Constants.RETRIEVE_ADDRID_TODELETE_IN_SQL + whereClause, String.class);
	...
	public void deleteProducerContractData(Portfolio port) {
		SqlParameterSource params = new BeanPropertySqlParameterSource(port);
		jdbcTemplate.update(Constants.DELETE_PRODUCER_PARTY_CONTRACT_SQL, params);
	}
	...
	private void deleteAddressData(Party party) {
		// String
		// whereClause=buildInClauseForClientAddress(party.getClientAddrList());
		// cscopt-1985 begin
		StringBuilder sb = new StringBuilder();
